...
_internal-repo: _build/warning
	[ -f _build/gtk/.git/refs/remotes/origin/wip/jimmac ] && rm -rf _build/gtk ||:
	[ -d _build/gtk ] && \
		(cd _build/gtk; git fetch origin; git reset --hard origin/gtk-3-24) || \
		git clone --depth=1 -b gtk-3-24 https://gitlab.gnome.org/GNOME/gtk.git _build/gtk
